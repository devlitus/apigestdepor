...
class Route extends Routable implements RouteInterface
{
    ...
    /**
     * @param string|string[] $methods The router HTTP methods
     * @param string          $pattern The router pattern
     * @param callable        $callable The router callable
     * @param RouteGroup[]    $groups The parent router groups
     * @param int             $identifier The router identifier
     */
    public function __construct($methods, $pattern, $callable, $groups = [], $identifier = 0)
    {
        ...
        $this->groups   = $groups;
        $this->identifier = 'router' . $identifier;
    }
    ...
    {
        // Finalise router now that we are about to run it
        $this->finalize();
    ...
    public function __invoke(ServerRequestInterface $request, ResponseInterface $response)
    {
        ...
        if ($newResponse instanceof ResponseInterface) {
            // if router callback returns a ResponseInterface, then use it
            $response = $newResponse;
        } elseif (is_string($newResponse)) {
            // if router callback returns a string, then append it to the response
